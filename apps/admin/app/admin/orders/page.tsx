...
function normalizeExecutionResult(response: JITExecutionResponse): ResultatAgregationJIT {
  ...
  return {
    ...
    cout_achat_estime: 0,
    marge_estimee: 0,
    statut: hasError ? "erreur" : hasAnyOrder ? "succes" : "aucune_commande",
...
export default function AdminOrdersPage() {
  ...
  useEffect(() => {
    if (lastLog?.date_execution && isToday(new Date(lastLog.date_execution)) && lastLog.nombre_commandes > 0) {
      setJitResult(normalizeLogResult(lastLog))
      setJitResultSource("execute")
    } else if (jitResultSource === "execute") {
      setJitResultSource(null)
    }
  }, [lastLog, jitResultSource])
  ...
  async function handleExecute() {
    ...
    try {
      ...
      if (execution.nombre_zones === 0) {
        setJitError("Aucune zone JIT active. Ouvrez /admin/zones puis créez une zone de livraison active.")
        ...
        setJitError(
          "JIT execute, mais aucune commande n'a ete verrouillee. Verifiez les zones actives et les adresses client geolocalisees."
        )
      } else {
      ...
      <main className="mx-auto max-w-7xl space-y-6 px-4 py-6 lg:ml-64 lg:px-8 lg:py-8">
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between gap-4 mb-6">
            <div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                {jitResultSource === "execute" && hasLockedOrdersToday ? (
                  <span className="inline-flex items-center rounded-full border border-green-200 bg-green-50 px-3 py-1 text-xs font-medium text-green-600">
          ...
          {hasLockedOrdersToday && (
            <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-700 flex flex-wrap items-center justify-between gap-3">
