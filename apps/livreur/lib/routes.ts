...
export const getDefaultDashboard = (_user: User | null) => "/livreur"
const LOGIN = "/login/livreur"
const PUBLIC = ["/login", "/verify", "/offline", "/politique-confidentialite"]
...
export function canAccessRoute({ pathname, user, isAuthenticated, isLoading }: { pathname: string; user: User | null; isAuthenticated: boolean; isLoading: boolean; searchParams?: URLSearchParams }): RouteAccessDecision {
  ...
  if (!(roles.has("LIVREUR") && !["ADMIN", "ADMIN_SUPER", "FOURNISSEUR", "PARENT"].some(value => roles.has(value)))) return { allowed: false, reason: "missing_role", redirectTo: LOGIN }
  return { allowed: true }
}
