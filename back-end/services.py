...
class CatalogueService:
    def __init__(self, session=None) -> None:
        ...
        self.product_dao: IProductDao = ProductDaoBD()

    # --- GESTION DE SESSION PRODUCTION ---
    def __enter__(self):
    ...
    def get_catalogue_complet(self) -> list[ProductResponseDTO]:
        ...
        catalogue_list: list[ProductResponseDTO] = []
        for p in products:
            catalogue_list.append(ProductResponseDTO(
...
class CommandeVocaleService:
    def __init__(self) -> None:
        ...
        self.commande_dao = None

    # --- GESTION DE SESSION PRODUCTION ---
    def __enter__(self):
        self.session = LocalSession()
        # On donne la MEME session au catalogue pour garantir l'intégrité de la transaction
        self.catalogue_service = CatalogueService(self.session)
        self.commande_dao: ICommandeVocaleDao = CommandeVocaleDaoBD()
        return self
    ...
    def traiter_commande(self, texte_transcrit: str, json_brut_gemini: str, langue: str) -> VoiceBasketResponseDTO:
        ...
        for item in items_gemini:
            ...
            if ligne_dto:
                self.catalogue_service.product_dao.decrement_stock(self.session, ligne_dto.product_id, ligne_dto.quantite_effective)
                
                self.commande_dao.create_ligne(
                    self.session, commande_entity.id, ligne_dto.product_id, ligne_dto.quantite_demandee,
                    ligne_dto.quantite_effective, ligne_dto.prix_unitaire, ligne_dto.sous_total,
        ...
        return VoiceBasketResponseDTO(
            status="success", transcription=texte_transcrit, langue_detectee=langue,
            produits_non_disponibles=produits_non_disponibles, lignes_panier=lignes_panier_dto,
            total_dh=round(total, 2), nombre_articles=len(lignes_panier_dto),
