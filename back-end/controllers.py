...
from dto import *
from services import AuthService, ProfileService,CommandeVocaleService, CatalogueService
import base64
...
def process_text_basket(body: TextBasketRequest):
    # Le "with" assure la sécurité production (ouverture/fermeture/rollback auto)
    with CommandeVocaleService() as voice_service:
        return voice_service.traiter_texte(body.texte)

@router_voice.post("/voice-basket", response_model=VoiceBasketResponseDTO)
def process_voice_basket(audio: UploadFile = File(...)):
    ...
    with CommandeVocaleService() as voice_service:
        return voice_service.traiter_audio(audio_b64, mime)

@router_catalogue.get("/catalogue", response_model=list[ProductResponseDTO])
def get_catalogue():
    with CatalogueService() as catalogue_service:
        return catalogue_service.get_catalogue_complet()